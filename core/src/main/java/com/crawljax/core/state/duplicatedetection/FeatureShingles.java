...
/**
 * FeatureShingles represents the features based on collecting shingles from a document. An 
 * example of a 2-shingle-collection of the string "abcd" => {"ab","bc","cd"}.
 */
public class FeatureShingles implements FeatureType {
	...
	/**
	 * @param size represents the size of a single shingle in the number of chunks.
	 * @param type the SizeType to be used for the shingles.
	 */
	 ...
	 * @param doc the document of the shingles
	 * @return shingles as an array
	 * @throws FeatureException Unknown feature-type or document to small for feature-generation.
	 */
	 ...
	 * @param type the type of string the shingles should be generated over.
	 * @param doc the document of the shingles
	 * @return shingles as an array
	 * @throws FeatureException Unknown feature-type or document to small for feature-generation.
	 */
	private String[] getFeatures(SizeType type, String doc) throws FeatureException {
		switch(type) {
			case CHARS: return this.getChars(doc);
			case WORDS: return this.getWords(doc);
			case SENTENCES: return this.getSentences(doc);
			default: throw new FeatureException("Unkown size-type " + type + " provided for the Shingle-feature."); 
		}
	}

	/**
	 * Splits the document into chars.
	 * @param doc the document to be split. Must contain more elements than the feature-type-size.
	 * @return array containing chars.
	 * @throws FeatureException the doc is too small to represent in this feature-type and size.
	 */
	private String[] getChars(String doc) throws FeatureException {
		String[] chars = doc.split("(?!^)");
		if (doc.length() < this.size) {
			throw new FeatureException("The size of the chars per feature is too large for this document. "
					+ "The document has a size of " + doc.length() + " char(s), feature needs " + size);
		}
	...
	private String[] getWords(String doc) throws FeatureException {
		...
		if (words.length < this.size) {
			throw new FeatureException("The size of the words per feature is too large for this document. "
					+ "The document has a size of " + words.length + " word(s), feature needs " + size);
		}
	...
	private String[] getSentences(String doc) throws FeatureException {
		...
		if (sentences.length < this.size) {
			throw new FeatureException("The size of the sentences per feature is too large for this document. "
					+ "The document has a size of " + sentences.length + " sentence(s), feature needs " + size);
		}
	 ...
	 * @param originalStrings the original DOM split-up in the parts to be shingled.
