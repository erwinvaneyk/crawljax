 ...
 * (near-)duplicates. 
 * 
 * To run the NearDuplicateDetection (NDD) classes correctly, the following criteria should be met:
 * - provide a threshold, which should adhere to the constraints of the specific implementation of the NDD.
 * - provide at least 1 featureType. This lets the NDD know how to hash the relevant content. 
 * 
 * Usage:
 * - Assume you have two documents (strings), which need to be evaluated, docA and docB
 * - Initialize a NearDuplicateDetection-implementation, and provide a threshold and feature(s).
 * - Use generateHash to calculate HashA and HashB for docA and docB
...
public interface NearDuplicateDetection {
	 ...
	 * @param doc a string, which should be larger than the feature-types.
